...
一个面向社团成员与审核员的一站式报销管理平台：发票 OCR 自动识别、报销表自动生成、多角色审核流程、草稿管理与权限申请。
...
### 账户体系
- 邮箱域名自动判定角色：`@cuhk.edu.cn` = 审核员，`@link.cuhk.edu.cn` = 社团成员
- 双重身份用户登录后自由选择进入界面
- JWT 认证 + bcrypt 密码哈希
...
- 面向**最终使用者**（社团成员 / 审核员）的说明文档：`功能手册.md` —— 用大白话逐页介绍平台"能做什么、不能做什么"，含报销流程、材料要求速查与 FAQ。
