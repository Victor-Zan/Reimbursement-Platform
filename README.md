...
> 平台内置的报销规则提示是依据**社联行政细则**整理的示例内容，其他部门/组织部署使用时**不要沿用**这部分规则，请按自身报销制度更新：

| 位置 | 内容 | 说明 |
|---|---|---|
| `frontend/src/components/RuleTips.tsx` | 提交向导三步的规则提示卡（发票抬头、额度、线下交付等） | 卡片底部已标注「社联行政细则示例」 |
| `报销人提交规则提示.md` | 社联行政细则整理全文 | 换成本组织细则后重写 |
| `backend/config.py`（`EXPECTED_BUYER_NAME` / `EXPECTED_TAX_ID`） | 发票抬头校验（OCR 自动核对） | 换成贵组织开票抬头与税号 |
| `template.xlsx` | 报销表模板 | 换成贵组织模板 |

平台本身（OCR、表单、审核流程、Excel 生成）与上述规则解耦，替换即可复用。
